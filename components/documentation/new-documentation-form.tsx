...
import { useCreateDocumentation } from "@/lib/queries/documentations";
import { User } from "@/lib/firebase/model/user";

type NewDocumentationFormProps = {
  user: User;
  onDataSaved: () => void;
};
...
export function NewDocumentationForm({
  user,
  onDataSaved,
}: NewDocumentationFormProps) {
  const { mutateAsync: createDocumentation } = useCreateDocumentation();
